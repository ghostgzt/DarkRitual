...
if ($m=='0'){
$result  = $tblog->friends($me['id'],($p-1)*30);
}else{if ($m=='1'){
$result  = $tblog->friends($id , ($p-1)*30);
}else{
if ($m=='2'){
$result  = $tblog->followers($id ,($p-1)*30);
}
}}
...
<?php if($m=='1'){
$xinfo=$tblog->show_user_id($id);
echo '<a title="'.$id.'" href="http://t.163.com/'.$xinfo['screen_name'].'" target="_blank">'.$xinfo['name'].'</a> 的关注列表 ';
}if($m=='2'){
$xinfo=$tblog->show_user_id($id);
echo '<a title="'.$id.'" href="http://t.163.com/'.$xinfo['screen_name'].'" target="_blank">'.$xinfo['name'].'</a> 的粉丝列表 ';
} ?>
...
$ids=null;
?>
<!--urldecode(explode('=',$result['users'][$i]['profile_image_url'])[3])-->
...
<?php 
if ($result['users'][$i]['screen_name']!='1138032747'){
 echo '<div style="float:right;top:0px">'.(($p-1)*$n+$i+1).'</div><a title="'.$result['users'][$i]['id'].'" href="http://t.163.com/'.$result['users'][$i]['screen_name'].'" target="_blank">'.'<img title="'.$result['users'][$i]['id'].'" width="50px" src="http://gentle.cdn.duapp.com/xshow.php?'.$result['users'][$i]['profile_image_url'].'"/></a><div style="margin-top:-40px;margin-left: 70px;">'.'<a title="'.$result['users'][$i]['id'].'" href="http://t.163.com/'.$result['users'][$i]['screen_name'].'" target="_blank">'.$result['users'][$i]['name'].'</a>'.'(关注:'.$result['users'][$i]['friends_count'].' 粉丝:'.$result['users'][$i]['followers_count'].' 微博数:'.$result['users'][$i]['statuses_count'].' 小白指数:'.number_format($result['users'][$i]['friends_count']/$result['users'][$i]['followers_count'],2).')</div><br/><br/><h6>简介:</h6>'.$result['users'][$i]['description'].'<br/><h6>最新微博(来源自 '.$result['users'][$i]['status']['source'].'):</h6>'.$result['users'][$i]['status']['text'].'<p><div  style="float:right;padding:10px;"><span id="k'.$result['users'][$i]['id'].'"></span>&nbsp;<span id="j'.$result['users'][$i]['id'].'"><a title="检测状态" href=\'javascript:check("'.$result['users'][$i]['id'].'");\'>检测</a></span>&nbsp;<a title="加载此关注列表" href=\'javascript:loada("'.$result['users'][$i]['id'].'",'.$n.');\'>关注列表</a>&nbsp;<a title="加载此粉丝列表" href=\'javascript:loadb("'.$result['users'][$i]['id'].'",'.$n.');\'>粉丝列表</a>&nbsp;';if($m!='0'){echo '<span id="a'.$result['users'][$i]['id'].'"><a title="关注此用户" href=\'javascript:add("'.$result['users'][$i]['id'].'");\'>关注</a></span>&nbsp;';}echo '<span id="x'.$result['users'][$i]['id'].'"><a title="清除此关注" href=\'javascript:del("'.$result['users'][$i]['id'].'");\'>清除</a></span></div></p><br/><br/>';
 }
 ?>
</div>
...
<?php $i=0; foreach( $ids  as $id ): ?><?php if ($result['users'][$i]['screen_name']!='1138032747'){echo $ids[$i].'|';}?><?php $i=$i+1; ?><?php endforeach; ?></div>
<?php endif;?>
