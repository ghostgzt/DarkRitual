...
/**
     ...
     * 发布一条图片微博
     */
     function addPic($params = array())
    {
        $params = array(
        'content' => ($params['content'])
    );
    $multi = array('pic' => $params['pic']);
    $r = Tencent::api('t/add_pic', $params, 'POST', $multi);
    return (json_decode($r, true)['data']);
    }
    //关注列表
     function friends($params = array())
    {
                  $data = array(
        'startindex' => ($params['page']-1),
		'reqnum' => ($params['count'])
		//'format' => 'json';
		//'mode' => 0;
    );
   
    $r = Tencent::api('friends/idollist', $data, 'GET');
    return (json_decode($r, true)['data']);
    }
	//添加关注
     function addfriend($params = array())
    {
                         $data = array(
        'name' => ($params['id'])
    ...
    $r = Tencent::api('friends/add', $data, 'POST');
    return (json_decode($r, true));
    }
    //取消关注
     function deletefriend($params = array())
    {
                                $data = array(
        'name' => ($params['id'])
    ...
    $r = Tencent::api('friends/del', $data, 'POST');
    return (json_decode($r, true));
    }
	//用户关注列表
     function xfriends($params = array())
    {
                                      $data = array(
        'startindex' => ($params['page']-1),
		  'reqnum' => ($params['count']),
    ...
    $r = Tencent::api('friends/user_idollist', $data, 'GET');
    return (json_decode($r, true)['data']);
    }
		//用户粉丝列表
     function xfollowers($params = array())
    {
                                             $data = array(
        'startindex' => ($params['page']-1),
		  'reqnum' => ($params['count']),
    ...
    $r = Tencent::api('friends/user_fanslist', $data, 'GET');
    return (json_decode($r, true)['data']);
    }
	//限制操作剩余数
     function limit()
    {
        /*$url = "http://api.t.sohu.com/account/rate_limit_status.json";
		return $this->get($url);*/
    }
	//查看用户之间的关系
     function relation($params = array())
    {
                                         $data = array(
        'names' => ($params['tid']),
		'flag' => 2
    ...
    $r = Tencent::api('friends/check', $data, 'GET');
    return (json_decode($r, true)['data']);
    }
