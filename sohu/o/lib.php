 ...
 *
 * send image
 * $w->upload( 'image test' , file_get_contents('http://www.sneakergo.com/data/afficheimg/1297414597384526958.jpg') );
 *
 */
...
class sohu 
{
 function __construct( $akey , $skey = '' ) 
 {
  $this->akey = $akey;
  $this->skey = $skey;
  $this->base = 'http://api.t.sohu.com/statuses/upload.xml';
  $this->curl = curl_init();
 ...
 function call_method( $method , $action , $args = '' ) 
 {
  ...
  //var_dump(curl_getinfo($this->curl), $this->postdata);ezdebug
  // clean post data
  $this->postInit();
